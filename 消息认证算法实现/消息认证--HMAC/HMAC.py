...
import sys
from HMAC_ui import Ui_Form
import copy

class HMAC(QWidget, Ui_Form):
    def __init__(self):
    ...
    def setupSignal(self):
        ...
        self.confirm.clicked.connect(self.calc_HMAC)
    ...
    def setupData(self):
        self.ipad = int("00110110"*(512//8), 16)
        self.opad = int("01011100"*(512//8), 16)
        self.K = [
    ...
    def padding_and_append_length(self, text_num):
        text ="%x"%(text_num)
        if(len(text) % 2 == 1):
            text = '0' + text
        length = len(text) * 4
    ...
    def hash512(self, text_num):
        nums = self.padding_and_append_length(text_num)#没有问题
    ...
    def calc_HMAC(self):
        ...
        if key and text:
            length = len(key)
            self.key = self.to_number(key)
            if length * 8 > 512:
                self.key = self.hash512(self.key)#得到512位的长度key，与b一样，不用填充
            elif length * 8 < 512:
                self.key = self.key << (512 - length * 8)

            self.msg = "%x"%self.to_number(text)
            if len(self.msg) % 2 == 1:
                self.msg = '0' + self.msg
            k = "%x"%(self.key ^ self.ipad)
            if len(k) % 2 == 1:
                k = '0' + k
            self.msg = int(k+self.msg, 16)
            S_in = self.hash512(self.msg)
            self.msg = int("%0128x"%(self.key ^ self.opad)+"%0128x"%S_in, 16)
            hmac = self.hash512(self.msg)
            self.textBrowser.setText("%0128x"%(hmac))
