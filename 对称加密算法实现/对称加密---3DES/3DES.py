...
import sys
from three_DES_ui import Ui_Form
import numpy as np 

class Three_DES(QWidget, Ui_Form):
    def __init__(self):
        super(Three_DES, self).__init__()
        self.setupUi(self)
        self.setupSignal()
        self.setupData()
    ...
    def save(self, index, bits):
        ...
        for i in range(16):
            for j in range(self.leftShiftNum[i]):
                ...
                right_key_28.pop(0)
            if index == 1:
                self.subKey1.append(self.permutate(left_key_28 + right_key_28, np.array(self.PC_2)-1))
            elif index == 2:
                self.subKey2.append(self.permutate(left_key_28 + right_key_28, np.array(self.PC_2)-1))
            elif index == 3:
                self.subKey3.append(self.permutate(left_key_28 + right_key_28, np.array(self.PC_2)-1))
    ...
    def saveKey(self):
        ...
        if key1 and key2 and key3:
            ...
            self.subKey2.clear()
            self.subKey3.clear()
            bits1 = self.string2bits(key1)
            bits2 = self.string2bits(key2)
            bits3 = self.string2bits(key3)
            self.save(1, bits1)
            self.save(2, bits2)
            self.save(3, bits3)
    ...
    def onEncryptionClicked(self):
        ...
        if plainText and len(self.subKey1):
            ...
            result = []
            for block in blocks:
                enText = self.encryption(self.decryption(self.encryption(block, self.subKey1), self.subKey2), self.subKey3)
                result.extend(enText)        
    ...
    def encryption(self, block, keys):#block是64位的0-1
        ...
        for i in range(16):
            ...
            right_block_48 = (np.array(extand_right_block) ^ np.array(keys[i])).tolist()
            right_block_32 = []
            for j in range(8):
    ...
    def onDecryptionClicked(self):
        ...
        if enText and len(self.subKey1):
            ...
            result = []
            for block in blocks:
                plainText = self.decryption(self.encryption(self.decryption(block,self.subKey3), self.subKey2), self.subKey1)
                result.extend(plainText)
            text = self.bits2string(result)
            self.textBrowser_2.setText(text)
    
    def decryption(self, block, keys):
        permutate_block = self.permutate(block, np.array(self.IP)-1)
        right_block = permutate_block[32:]
        left_block = permutate_block[:32]
