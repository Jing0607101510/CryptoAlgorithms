...
import sys
from RSA_ui import Ui_verify
import copy
import random

class RSA(QWidget, Ui_verify):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        self.setupData()
    ...
    def setupSignal(self):
        ...
        self.verification.clicked.connect(self.varify_ds)
    ...
    def genKey(self):
        ...
        self.textBrowser.setText("公钥：(%x, %x)       私钥：(%x, %x)"%(self.n, self.e, self.n, self.d))
    ...
    def gen_ds(self):
        ...
        if plain_text:
            ...
            encry_text = self.fast_exp_mode(hash, self.d, self.n)
            self.textBrowser_2.setText("%x"%encry_text)
            

    def varify_ds(self):
        plain_text = self.textEdit_2.toPlainText()
        ds = self.textEdit_3.toPlainText()
        if ds and plain_text:
            hash = self.start_hash(plain_text)
            ds = int(ds, 16)
            decry_hash = self.fast_exp_mode(ds, self.e, self.n)
            if hash == decry_hash:
