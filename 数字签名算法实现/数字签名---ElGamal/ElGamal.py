...
class ElGamal(QWidget, Ui_Form):
    ...
    def setupSignal(self):
        ...
        self.gen_random.clicked.connect(self.genRandom)
    ...
    def setupData(self):
        ...
        self.random_K = self.find_random_K(self.prime)
        self.lineEdit_8.setText(str(self.random_K))
        self.K = [
    ...
    def find_random_K(self, n):
        while 1:
            e = random.randint(0, n)#e随机产生
            x,y,r =  self.extend_gcd(e, n-1)#e与ln需要互质
            if r == 1:#e与ln就是互质了
                return e
    ...
    def gen_ds(self):
        ...
        if plain_text:
            hash = self.start_hash(plain_text)#整数一个
            S1 = self.fast_exp_mode(self.prime_root, self.random_K, self.prime)
            K_1 = self.get_inverse(self.random_K, self.prime-1)
            S2 = (K_1 * ((hash - (self.xa*S1)%(self.prime-1)))%(self.prime-1))%(self.prime-1)
            self.textBrowser_2.setText("(%x, %x)"%(S1, S2))

    def varify_ds(self):
        recept_hash = self.textEdit_2.toPlainText()
        ds = self.textEdit_3.toPlainText().replace("(", "")
        ds = ds.replace(")", "")
        recept_DS_S1, recept_DS_S2 = ds.split(",")
        if recept_hash and recept_DS_S1 and recept_DS_S2:
            recept_DS_S1 = int(recept_DS_S1, 16)
            recept_DS_S2 = int(recept_DS_S2, 16)
            recept_hash = int(recept_hash, 16)
            V1 = self.fast_exp_mode(self.prime_root, recept_hash, self.prime)
            V2 = (self.fast_exp_mode(self.public_a, recept_DS_S1, self.prime) * self.fast_exp_mode(recept_DS_S1, recept_DS_S2, self.prime))%self.prime
            if V1 == V2:
                self.lineEdit_9.setText("Yes!!!")
            else:
                self.lineEdit_9.setText("No!!!")
