...
import sys
from DSA_ui import Ui_Form
import random
import copy

class DSA(QWidget, Ui_Form):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        self.setupData()
    ...
    def setupData(self):
        ...
        self.prime_1_factor = int(self.line_q.text())
        self.g = int(self.line_g.text())
        self.x = random.randint(1, self.prime_1_factor)
        self.line_x.setText(str(self.x))
        self.y = self.gen_pub_key(self.x)
        self.line_y.setText(str(self.y))
    ...
    def gen_pub_key(self, x):
        return self.fast_exp_mode(self.g, x, self.prime)
    ...
    def gen_ds(self):
        ...
        if plain_text:
            k = random.randint(1, self.prime_1_factor)
            r = self.fast_exp_mode(self.g, k, self.prime) % self.prime_1_factor
            hash = self.start_hash(plain_text)
            s = (self.get_inverse(k, self.prime_1_factor)*(hash%self.prime_1_factor+(self.x*r)%self.prime_1_factor))%self.prime_1_factor
            self.textBrowser_1.setText("%x"%hash)
            self.textBrowser_2.setText("(%x, %x)"%(r, s))
    ...
    def varify_ds(self):
        ...
        ds = ds.replace(")", "")
        r, s = ds.split(",")
        if plain_text and r and s:#这些都是字符串
            r = int(r, 16)
            s = int(s, 16)
            
            w = self.get_inverse(s, self.prime_1_factor)
            hash = self.start_hash(plain_text)
            u1 = ((hash % self.prime_1_factor) * w) % self.prime_1_factor
            u2 = (r * w) % self.prime_1_factor
            v = (self.fast_exp_mode(self.g, u1, self.prime)*self.fast_exp_mode(self.y, u2, self.prime)%self.prime)%self.prime_1_factor
            if v == r:
