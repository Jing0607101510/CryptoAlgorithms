...
import sys
from Diffie_Hellman_ui import Ui_Form
import random
import numpy as np

class Diffie_Hellman(QWidget, Ui_Form):
    def __init__(self):
        super(Diffie_Hellman, self).__init__()
        self.setupUi(self)
        self.setupSignal()
        self.setupData()
    ...
    def setupData(self):
        ...
        self.xa = random.randint(1, 1000000000000)
        self.xb = random.randint(1, 1000000000000)
        self.public_a = self.gen_pub_key(self.xa)
        self.public_b = self.gen_pub_key(self.xb)
        self.key = self.calc_key()
        self.lineEdit_3.setText(str(self.xa))
        self.lineEdit_4.setText(str(self.xb))
        self.lineEdit_5.setText(str(self.public_a))
        self.lineEdit_6.setText(str(self.public_b))
        self.lineEdit_7.setText(str(self.key))
        self.subKey = []
    ...
    def saveKey(self):
        self.subKey.clear()
        bits = list('{0:b}'.format(self.key))
        bits = [int(n) for n in bits]
        if len(bits) < 64:
            bits += [0] * (64 - len(bits))
        elif len(bits) > 64:
            bits = bits[:64]
    ...
    def calc_key(self):
        return self.fast_exp_mode(self.public_b, self.xa, self.prime)
